                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-checkbox" class="table table-striped table-bordered bulk_action" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <td><input type="checkbox" class="checkboxtest" value="{{ $user->id }}"></td>
                        <td>{{ $key+ 1}}</td>  
                        <td><a href="{{ route('admin.buyer.detail', $user->id )}}">{{ $user->name }}</a></td>
                        <td>{{ $user->phone }}</td>
                        <td>{{ $user->email }}</td>
                        {{-- E:\ZnnServer\shoppyjapan\storage\app\public\upload\brand_images\6284e47fe55aewomen.jpg --}}
                        <td>
                          @if($user->address1 != NULL)
                          {{ $user->address1 }}, {{ $user->address2 }} {{ $user->address3 }}  {{ $user->postcode }} {{ $user->city }}, {{ $user->state }}</td>
                          @endif
                        </tr>
                        @endforeach
                      </tbody>
